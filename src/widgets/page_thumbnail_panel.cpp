...
PageThumbnailPanel::PageThumbnailPanel(PdfViewer *viewer, QWidget *parent)
    : QWidget(parent), pdfViewer_(viewer), thumbnailList_(nullptr), layout_(nullptr) {
  setupUI();
  
  if (pdfViewer_) {
    connect(pdfViewer_, &PdfViewer::pdfLoaded, this, &PageThumbnailPanel::onPdfLoaded);
    connect(pdfViewer_, &PdfViewer::pdfClosed, this, &PageThumbnailPanel::onPdfClosed);
    connect(pdfViewer_, &PdfViewer::pageChanged, this, &PageThumbnailPanel::onPageChanged);
  }
}
...
void PageThumbnailPanel::setupUI() {
  ...
  connect(thumbnailList_, &QListWidget::itemClicked, 
          this, &PageThumbnailPanel::onItemClicked);
...
void PageThumbnailPanel::refreshThumbnails() {
  generateThumbnails();
}
...
QPixmap PageThumbnailPanel::renderThumbnail(int pageIndex) {
  ...
  return pixmap.scaled(THUMBNAIL_WIDTH, THUMBNAIL_HEIGHT, 
